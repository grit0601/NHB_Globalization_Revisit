...
# load packages with "p_load", which will automatically install packages that not exist in the system.
...
The census data of China are downloaded from the national statistics bureau (). All these data are first preprocessed and stored in *.RData*.

Files downloaded include:

-   A0401_CNH_Census7_Age_education_edited.xls
-   A0105_region_age_sex_edited.xls
...
# save(df_cn_census, df_cn_census_geo, ChinaMap, China_border_line, colour, file = here::here("Re_Analysis_CHN.RData"))

# load the data from Ruggeri 2022, it includes three variables:
# "dat_filter", "dat_item", "dat_unique"
load(here::here("1_2_data_files.RData"))

# load data for Chinese sample, it includes five variables:
# "China_border_line", "ChinaMap", "colour","df_cn_census", "df_cn_census_geo"
# "China_border_line", "ChinaMap" were reproduced from http://xzqh.mca.gov.cn/data/, now the official standard map was move to http://bzdt.ch.mnr.gov.cn/, the approval number is GS（2019）1651;
...
# import data for Nigeria, it includes two variable:
# "df_ng_wb": data extracted from World Bank, df_ng_wb <- bruceR::import('Data_extracte_WorldBankData_Series_Data.csv')
...
# save(df_ng_map, df_ng_wb, file = here::here("Re_Analysis_NG.RData"))

# load data for US, which include one variable "df_us_map"
load(here::here("Re_Analysis_USA.RData"))    # for geo plots
# the data files below are also needed:
# "ACSST5Y2021.S0101-2023-02-08T075713.csv"
# "ACSDT5Y2021.B15001-2023-02-08T135043.csv" 
# "DECENNIALPL2020.P1-2023-02-08T080125.csv"

# census_api_key("", install = TRUE, overwrite=TRUE) # data downloaded from gov
...
fig1b <- ggplot(data= df_cn_census_age, aes(x=ageBins, y=ifelse(Sex=="Male", -Proportion, Proportion), fill=Sex)) +
  ...
  geom_line(data = df_cn_ruggeri2022_age, aes(x=ageBins_pyr,
                                      ...
                                      group=Sex,color="Ruggeri2022"), linewidth=1, inherit.aes = FALSE) +
...
To make it comparable, we re-code both and used the following schema: "noschool", "primary", "secondary" (include both middle and high school), "Technical college", "Bachelor", "master" (include graduate and MBA), and "PhD".
...
# Add population data for "Hong Kong SAR", "Macau SAR", and "Taiwan"
new_rows <- data.frame(
      Regions_CN = c("香港", "澳门", "台湾"),
      Regions_EN = c("Hong Kong SAR", "Macau SAR", "Taiwan"),
      N = c(7474200, 683218, 23561236 ), 
      Proportion = c(0.51778411, 0.04733074, 1.63223268 ),
      PropBins = c("< 0.9%", "< 0.9%", "1~4.9%"),
      Colors = c(1, 1, 2) 
      )

df_cn_census_geo_adults_all <-   rbind(df_cn_census_geo_adults_all, new_rows)
df_cn_census_geo_adults_all
...
df_cn_geo_4 <- dplyr::left_join(df_cn_geo_3, colour,by= "province")
# head(colour)

# plot the main part
fig1d_1 <- ggplot(data = df_cn_geo_4) +
geom_sf(aes(geometry = geometry,fill = colour_psySample), size = 0.001, show.legend = TRUE) +
# geom_sf(data = dat_map_4, aes(geometry = geometry)) +
...
fig1e_1 <- ggplot(data = df_cn_geo_4) +
geom_sf(aes(geometry = geometry,fill = colour_census), size = 0.001, show.legend = TRUE) +
# geom_sf(data = dat_map_4, aes(geometry = geometry)) +
coord_sf(default_crs = sf::st_crs(4326)) +
scale_fill_manual(
# "class",
...
All data used here are downloaded from <https://data.census.gov>

## Sex ratio

```{r US data Sex}
## import data downloaded from: https://data.census.gov
...
# extract data from Ruggeri et al. 2022
...
fig3d <- ggplot(data = df_us_map, aes(fill = df_us_census_geo$Colors)) + 
  ...
  labs(
    title = "Geographical distribution (Census)",
    ) + 
  theme(
        plot.title = element_text(
          color = "black",
