...
# According to the above link, register the website and provide additional information; Download the required data compression packages in sequence (a total of 77 compression packages).
# Among them, the required file package names are as follows: 11. After downloading, extract them one by one to the corresponding named folder
#
# Standard number: GB/T 13923-2022
# Chinese Standard Name: Classification and Code of Basic Geographic Information Features
# English standard name: Classification and codes for fundamental geographic information feature
...
layer_cn <- c('G51.gdb','F51.gdb','E50.gdb','D50.gdb',
              'C50.gdb','B50.gdb','A49.gdb','B49.gdb',
              'C49.gdb','D49.gdb','E49.gdb')
dat_map <- data.frame()
for (i in layer_cn){
  dat <- sf::read_sf(i,layer = 'BOUL')
...
df_cn_geo_3 <- rbind(df_cn_geo_2, dat_map_4[, c("Name_Province", "geometry")])
df_cn_geo_3 <- rename(df_cn_geo_3, province = Name_Province)
...
for(i in seq_along(replacements)) {
  df_cn_geo_3$province <- gsub(names(replacements)[i], replacements[[i]], df_cn_geo_3$province)
}
