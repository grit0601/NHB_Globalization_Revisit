#---------------------------------------------------#
### the code was largely learnt from: https://mp.weixin.qq.com/s/xQ_jFl3kRZkSQ8NtglFYMg &  https://mp.weixin.qq.com/s/cX9tlGbGiPh27l4ausTfbQ
#
# Data information description: https://datav.aliyun.com/portal/school/atlas/area_selector
# We downloaded the data and manually changed the Chinese name of the data file from 'People's Republic of China. json' to the English name 'china. json'.
# The data is for learning and research purposes only.
#---------------------------------------------------#

china <- geojsonsf::geojson_sf('china.json') 
china_name <- rjson::fromJSON(file="china.json",simplify=T) 
...
ChinaMap <- china %>% dplyr::select(-name) %>%
  mutate(province=name)
